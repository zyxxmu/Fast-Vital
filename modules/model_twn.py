...
from .nnUtils_for_twn import BinLinear,BinConv2d
...
class TWNMDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        super(TWNMDNet, self).__init__()
        self.K = K
        self.layers = nn.Sequential(OrderedDict([
                ...
                ('conv2', nn.Sequential(BinConv2d(96, 256, kernel_size=5, stride=2),
                                        nn.ReLU(inplace=True),
                                        nn.LocalResponseNorm(2),
                                        nn.MaxPool2d(kernel_size=3, stride=2))),
                ('conv3', nn.Sequential(BinConv2d(256, 512, kernel_size=3, stride=1),
                                        nn.ReLU(inplace=True))),
                ('fc4',   nn.Sequential(BinLinear(512 * 3 * 3, 512),
                                        nn.ReLU(inplace=True))),
