...
class VGG_RandomPrune(nn.Module):
    def __init__(self, Prunerate=1):
        super(VGG_RandomPrune, self).__init__()
        self.Prunerate = Prunerate
        self.layers = self._make_layers([64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M', 512, 512, 512, 'M'])
        self.classifier = nn.Linear(int(512 * Prunerate), 10)
...
class VGG_Vital(nn.Module):
    def __init__(self, model_path=None, K=1, Prunerate=1):
        super(MDNet_RandomPrune, self).__init__()
        self.K = K
        self.Prunerate=Prunerate
        self.layers = nn.Sequential(OrderedDict([
                ...
                ('conv2', nn.Sequential(nn.Conv2d(int(96*Prunerate), int(256*Prunerate), kernel_size=5, stride=2),
                                        ...
                                        nn.LocalResponseNorm(2),
                                        nn.MaxPool2d(kernel_size=3, stride=2))),
                ('conv3', nn.Sequential(nn.Conv2d(int(256*Prunerate), 512, kernel_size=3, stride=1),
                                        nn.ReLU(inplace=True))),
        ...
        if model_path is not None:
            if os.path.splitext(model_path)[1] == '.pth':
                ...
                self.load_vgg_model(model_path)
    ...
    def load_vgg_model(self, model_path):
        states = torch.load(model_path)
        filterstate = {
                        '0':96,
                        '1':256,
        ...
        for i in range(3)：
            oriweight = states['conv' +str(i) + '.weight']
            oribias = states['conv'+ str(i) + '.bias']

            select_index = random.sample(range(0, filterstate[str(i)]-1), int(filterstate[str(i)]*self.Prunerate))
            select_index.sort()
            if last_select_index is not None:
                ...
                #print(self.layers[i][0].weight.data.size())
                #print(oriweight.size())
                for index_k, k in enumerate(select_index):
